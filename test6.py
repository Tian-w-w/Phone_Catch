...
# 配置区
# ──────────────────────────────────────────────
OLLAMA_BASE_URL = "http://10.19.205.173:11434/v1"   # Ollama 默认 HTTP
...
请分析图片，判断是否存在手机。
【判定流程】：
1. 在脑中检查五个条件（A-E），不要输出过程。
2. 执行最终判定：
   - 若 (条件C = true 且 匹配总数 >= 2) OR (匹配总数 >= 3 且 证据确凿) -> "YES"
   - 其余所有情况 -> "NO"
【输出要求】：
只返回如下最简 JSON，绝对不得输出任何其他内容：
{"is_phone": "YES"}  或  {"is_phone": "NO"}
/no_think
"""
...
def parse_result(raw: str, filename: str = "") -> str:
    """
    从模型输出中提取 is_phone，返回 YES/NO。
    处理链路：剥离 <think> 块 → 去 markdown 栅栏 → JSON 解析 → 文本降级匹配
    """
    # 1. 剥离 Qwen3 思考块 <think>...</think>（含跨行情况）
    ...
    # 4. 降级：文本关键词匹配
    upper = cleaned.upper() or raw.upper()
    if '"IS_PHONE": "YES"' in upper or "'IS_PHONE': 'YES'" in upper or '"IS_PHONE":"YES"' in upper:
        return "YES"

    # 5. 兜底：记录完整原始输出，方便定位问题
    tag = f"[{filename}] " if filename else ""
    logger.warning(
        f"{tag}JSON 解析失败，原始输出（全量）↓\n"
        f"{'─'*40}\n{raw}\n{'─'*40}"
    )
    return "NO"


def detect_phone(image_path: Path) -> tuple[str, float]:
    """
    调用 Qwen3-VL 判断单张图片是否含手机。
    返回 (YES/NO, 耗时秒数)。
    """
    import time
    ...
    payload = {
        ...
        "temperature": 0,
        "max_tokens": 2048,   # reasoning 链需要空间，content(JSON) 在其后才输出
        "think": False,       # 部分 Ollama 版本支持，尽量关掉
        "stream": False,
    }
    ...
    # ── 调试：打印完整 API 响应，帮助定位空输出原因 ──
    logger.debug(f"[{image_path.name}] 完整响应：{json.dumps(resp_json, ensure_ascii=False)}")

    msg = resp_json["choices"][0]["message"]

    # Ollama Qwen3：思考内容在 reasoning 字段，最终答案在 content
    content   = (msg.get("content")   or "").strip()
    reasoning = (msg.get("reasoning") or "").strip()

    # 两者都空时，打印完整 message 结构帮助排查
    if not content and not reasoning:
        finish_reason = resp_json["choices"][0].get("finish_reason", "unknown")
        usage         = resp_json.get("usage", {})
        logger.warning(
            f"[{image_path.name}] 模型返回空内容！\n"
            f"  finish_reason : {finish_reason}\n"
            f"  usage         : {usage}\n"
            f"  message keys  : {list(msg.keys())}\n"
            f"  message raw   : {json.dumps(msg, ensure_ascii=False)}"
        )

    raw = content if content else reasoning
    return parse_result(raw, filename=image_path.name), round(elapsed, 1)
...
def run_folder(folder: str, workers: int = 1) -> None:
    ...
    def _process(idx_path: tuple[int, Path]) -> tuple[int, Path, str, float]:
        idx, path = idx_path
        result, elapsed = detect_phone(path)
        return idx, path, result, elapsed
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                _, _, result, elapsed = future.result()
            ...
            logger.info(f"[{idx:05d}/{total}] {result:<3}  {elapsed:>6.1f}s  {path.name}")

    no_count   = total - yes_count - err_count
    wall_total = time.perf_counter() - wall_start
    avg        = wall_total / (total - err_count) if (total - err_count) > 0 else 0
    logger.info("-" * 60)
    logger.info(
        f"完成 | 共 {total} 张 | "
    ...
    logger.info(
        f"总耗时：{wall_total:.1f}s  |  平均每张：{avg:.1f}s"
    )
    logger.info("=" * 60)
